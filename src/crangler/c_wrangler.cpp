...
void c_wranglert::configure_functions(const jsont &config)
{
  ...
  if(!functions.is_object())
    throw deserialization_exceptiont("functions entry must be map");

  for(const auto &function : to_json_object(functions))
  {
    const auto &function_name = function.first;
    const auto &items = function.second;
  ...
  std::ostream &out)
{
    ...
    declaration.is_function() && name_opt.has_value() && declaration.has_body())
  {
    ...
    if(f_it != config.functions.end())
    {
      ...
      if(f_it->second.stub.has_value())
      {
        // replace by stub
        out << f_it->second.stub.value();
      }
      else
      {
        if(f_it->second.remove_static)
        {
